...
export default function SolarSystemCanvas() {
  ...
  useEffect(() => {
    ...
    // 🔹 4 PLANETAS + COR + ÂNGULO DE GIRO + LUAS
    const planets = [
      { 
    ...
    // 🔹 MAPA DE IMAGENS DE FUNDO (PLANETAS + SOL)
    ...
    // Ajuste os caminhos conforme seus arquivos (public/ ou import)
    ...
    // 🔹 foco pode ser null, "sun", objeto planeta OU objeto lua ⭐
    ...
    function draw() {
      ...
      if (focusedTarget) {
        ...
        } else if (focusedTarget.planet && focusedTarget.planet.name) {
    ...
    function animate() {
      ...
      void atTarget; // não usados, mas deixei se quiser debugar depois
      void almostAtTarget;

      // 🔹 GIRO: para um pouco antes de chegar
      if (!focusedTarget) {
        sunSpinAngle += 0.02;
        for (const p of planets) {
          p.spinAngle += 0.02;
        ...
        if (focusedTarget === "sun") {
          ...
          for (const p of planets) {
            if (p === focusedTarget || focusedTarget.planet === p) {
              if (!almostAtTarget) {
                p.spinAngle += 0.2;
              }
            }
